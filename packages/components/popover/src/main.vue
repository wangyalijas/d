<template>
  <span>
    <transition
      ...
      <div
        class="w-popover w-popper"
        :class="[popperClass, content && 'w-popover--plain']"
        ref="popper"
      ...
      >
        <div class="w-popover__title" v-if="title" v-text="title"></div>
        <slot>{{ content }}</slot>
      </div>
    </transition>
...
export default {
  ...
  mounted() {
    ...
    // 可访问性
    if (reference) {
      addClass(reference, 'w-popover__reference');
