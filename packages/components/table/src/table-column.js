...
export default {
  ...
  created () {
      ...
      if (value !== undefined) {
        if (prop === 'renderHeader') {
          if (type === 'selection' && column[prop]) {
            console.warn('[Warn][TableColumn]Selection column doesn\'t allow to set render-header function.')
          } else {
    ...
    if (this.renderHeader) {
      console.warn('[Warn][TableColumn]Comparing to render-header, scoped-slot header is easier to use. We recommend users to use scoped-slot header.')
    }
    ...
    column.renderCell = function (h, data) {
      ...
      return _self.showOverflowTooltip || _self.showTooltipWhenOverflow
        ? <div class="cell w-tooltip" style={ {width: (data.column.realWidth || data.column.width) - 1 + 'px'} }>{ renderCell(h, data) }</div>
  ...
  mounted () {
    ...
    if (this.$scopedSlots.header) {
      if (this.type === 'selection') {
        console.warn('[Warn][TableColumn]Selection column doesn\'t allow to set scoped-slot header.')
      } else {
