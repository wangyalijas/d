<template>
  <label
    class="w-checkbox"
    :class="[
      border && checkboxSize ? 'w-checkbox--' + checkboxSize : '',
  ...
  >
    ...
    <span class="w-checkbox__label" v-if="$slots.default || label">
      <slot></slot>
...
export default {
  ...
  computed: {
    ...
    checkboxSize () {
      const temCheckboxSize = this.size || this._wFormItemSize || (this.$ELEMENT || {}).size
      return this.isGroup
