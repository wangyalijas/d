...
const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => t(`w.datepicker.weeks.${week}`)),
    dayNames: weeks.map(week => t(`w.datepicker.weeks.${week}`)),
    monthNamesShort: months.map(month => t(`w.datepicker.months.${month}`)),
    monthNames: months.map((month, index) => t(`w.datepicker.month${index + 1}`)),
    amPm: ['am', 'pm']
  }
}
