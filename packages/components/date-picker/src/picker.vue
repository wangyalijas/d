<template>
  ...
  <div
    class="w-date-editor w-range-editor w-input__inner"
    :class="[
      'w-date-editor--' + type,
      pickerSize ? `w-range-editor--${ pickerSize }` : '',
    ...
    v-clickoutside="handleClose"
    v-else>
    <i :class="['w-input__icon', 'w-range__icon', triggerClass]"></i>
    <input
      autocomplete="off"
    ...
    <slot name="range-separator">
      <span class="w-range-separator">{{ rangeSeparator }}</span>
    </slot>
    <input
      autocomplete="off"
    ...
    <i
      ...
      v-if="haveTrigger"
      :class="[showClose ? '' + clearIcon : '']"
      class="w-input__icon w-range__close-icon">
    </i>
  </div>
</template>
...
export default {
    ...
    pickerVisible (val) {
      ...
      if (val) {
        ...
        if (this.validateEvent) {
          this.dispatch('WFormItem', 'w.form.blur')
        }
        this.$emit('blur', this)
        this.blur()
      }
  ...
  computed: {
    ...
    triggerClass () {
      return this.prefixIcon || (this.type.indexOf('time') !== -1 ? 'w-icon-time' : 'w-icon-date')
    },
